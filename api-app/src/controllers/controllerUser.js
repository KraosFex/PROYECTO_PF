...
const getUsersByName = async (req, res, next) => {
  ...
  try {
    const user = await User.find({
      ...
      .populate({ path: "courses.course", ref: "Course" })
      .populate({
        path: "courses.course",
        ref: "Course",
        populate: { path: "lessons.lesson", ref: "Lesson" },
...
const overallPosition = async (req, res) => {
  ...
  try {
    ...
    const sorted = allUsers
      .filter((element) => element.courses.length > 0)
      .slice(0, 5)
      .sort((a, b) => {
        return (
          a.courses?.map((c) => {
...
const isPremium = async (req, res) => {
  ...
  try {
    ...
    res.send({
      info: "Felicidades ahora eres Premium",
      updateUser,
      success: true,
    });
  } catch {
    res
      .status(500)
      .send({ info: "Error al realizar esta accion", success: false });
  }
};
