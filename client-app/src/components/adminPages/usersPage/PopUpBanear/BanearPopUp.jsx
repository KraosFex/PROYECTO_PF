        ...
        >
          <div className={style.inputs_container}>
            <input
              type="date"
              name="fecha"
              defaultValue={`${año}-${mes}-${dia - 1}`}
              min={`${año}-${mes}-${dia}`}
            ></input>
          </div>
          {fecha !== "" && (
            <input className={style.boton} type="submit" value="Listo"></input>
          )}
        </form>
        {error.successful ? (
          <label className={style.label2}>
            Listo, tu usuario fue baneado hasta {fecha}
          </label>
        ) : null}
        {error.error ? (
          <label className={style.label}>
            <p className={style.p}>Hubo un error con la ruta</p>
            <p className={style.p}>
              {" "}
              Intenta mas tarde o comunicate con los administradores
            </p>
          </label>
        ) : null}
      </div>
