 ...
 const handleSubmit = async (event) => {
   ...
   if (!isObjectEmpty(error) || isObjectEmpty(input)) {
     ...
     setError(validator("URL", {
       ...
       event.preventDefault()
       const response = await dispatch(editImage(input.URL))
    ...
    <div className={style.overlay}>
      <div className={style.pop_up}>
        <div className={style.close_and_title}>
          <a href='#' id="btn_close_popup" className={style.btn_close_popup} onClick={() => popUpFunction("image", false)}><AiFillCloseCircle className={style.icon} /></a>
          <h2>Cambia la imagen de tu perfil</h2>
        </div>
        <h4>Introduce una URL de una imagen que quieras que aparezca en tu perfil.</h4>
