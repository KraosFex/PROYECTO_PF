...
import NavBarUser from "../home/navbarUser/navBarUser.jsx"
...
export default function CardD({ navbar }) {
        ...
        <div><NavBarUser />
            <div className={style.Container}>
                ...
                <div className={style.progreso}>
                    {Curso.clases.map((e) => <div className={style.ClasP} key={e.id}>{e.isCompleted ? <input checked type="radio" readOnly /> : <input key={e.id} type="radio" disabled />}{e.title}</div>)}
                    {Curso.clases.map((e) => <div className={style.ClasP} key={e.id}>{e.isCompleted ? <input checked type="radio" readOnly /> : <input type="radio" disabled />}{e.title}</div>)}
                    {Curso.clases.map((e) => <div className={style.ClasP} key={e.id}>{e.isCompleted ? <input checked type="radio" readOnly /> : <input type="radio" disabled />}{e.title}</div>)}
                </div>
