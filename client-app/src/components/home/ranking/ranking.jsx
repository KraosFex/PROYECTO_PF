...
import { getranking,getrankingid } from "../../../../redux/actions";
import style from "./ranking.module.css";

export default function Ranking() {
    const { topten, user } = useSelector((store) => store);
    const dispatch = useDispatch();
    if (topten.length === 0) { getranking()(dispatch) }
    if (user.id) { getrankingid(id)(dispatch) }
    let usuarios = topten
    ...
    return (<div className={style.All}>
        ...
        <div className={style.Contenedor}>
            ...
            {user.top ? <div className={style.Contenedor3}>
                <div className={style.infouser}>
                    <label>{user.top}.{user.username}</label>
                    <img className={style.trofeo} alt="" src=" https://images.emojiterra.com/google/noto-emoji/v2.034/512px/1f3c6.png" />
                </div>
            </div> : null}
        </div>
    </div>)
