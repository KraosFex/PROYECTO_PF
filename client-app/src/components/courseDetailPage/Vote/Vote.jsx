...
export default function Stars({idCurso,idUser}) {
    ...
    function Click() {
        ...
        let max = 5;
        for (let i = 0; i < 5; i++) {
            if (inputs[i].checked) { max = inputs[i].id.split("rc")[1] }
            if (max === 0 || inputs[i].id.split("rc")[1] <= max) { labels[i].children[0].className.baseVal = "starselect" }
            else { labels[i].children[0].className.baseVal = "star" }
        }
    }
    function SendVote(){
        let inputs = document.getElementsByName("rating-star");
    ...
    return (
        <div className="Conteiner_rating">
            <input type="radio" name="rating-star" onChange={(e) => { Click(e) }} className="rating" id="rc1" />
            <input type="radio" name="rating-star" onChange={(e) => { Click(e) }} className="rating" id="rc2" />
            <input type="radio" name="rating-star" onChange={(e) => { Click(e) }} className="rating" id="rc3" />
            <input type="radio" name="rating-star" onChange={(e) => { Click(e) }} className="rating" id="rc4" />
            <input type="radio" name="rating-star" onChange={(e) => { Click(e) }} className="rating" id="rc5" />
