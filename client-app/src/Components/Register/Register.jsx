...
function Validate(input) {
  ...
  if (input.name !== "" && input.password !== "" && input.email !== "" && input.password2 !== "") {
    if (!/^(?=.*\d)(?=.*[a-zA-Z]).{8,20}/.test(input.password)) { errores.password = "Must have at least one number, one letter and 8 characters" }
...
function Register() {
  const [input, setInput] = React.useState({
    name: "", password: "", password2: "", email: "", Image: ""
  });
  ...
  const Visible = () => {
    ...
    if (password.type === "password") { password.type = "text"; password2.type = "text" }
    else { password.type = "password"; password2.type = "password" }
  }
  const Changes = (e) => {
    ...
    <div>
      <div className={style.Container}>
        <form className={style.form} onSubmit={(e) => Submitiar(e)} id="formul">
          ...
          <div className={style.divs}>
            Password : <label className={style.beye} onClick={(e) => Visible(e)}><img alt="1" className={style.eye} src="https://cdn-icons-png.flaticon.com/512/58/58976.png" /></label>
          ...
          <div className={style.divimg}>
            Repeat password:
            <input id="pasw2" name="password2" type="password" onChange={(e) => Changes(e)} placeholder="Repeat password..." className={style.inputs} required />
          </div>
          ...
          <div className={style.btns}>
            ...
            {errores.good ? <label className={style.send2} onClick={() => Reset()}>Clear</label> : null}
          </div>
