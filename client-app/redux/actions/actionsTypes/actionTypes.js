export const SET_COURSES = 'SET_COURSES';
export const SET_SHOWEDCOURSES = 'SET_SHOWEDCOURSES';
export const SET_VALIDATEUSER = "SET_VALIDATEUSER";
export const SET_THEME = "SET_THEME";
export const LOGOUT = "LOGOUT";
export const SET_UPDATEUSER = "SET_UPDATEUSER";
export const SET_ALLUSERS = "SET_ALLUSERS";
export const SET_SHOWEDUSERS = "SET_SHOWEDUSERS";
export const SET_RANKING = "SET_RANKING";
export const SET_ARROW_DIRECTION = "SET_ARROW_DIRECTION";
export const SET_ARROW_UPDOWN = "SET_ARROW_UPDOWN";
export const SET_ARROW_COURSE = "SET_ARROW_COURSE";
export const SET_PAGINATE_USERS = "SET_PAGINATE_USERS";
export const SET_PAGINATE_COURSES = "SET_PAGINATE_COURSES";
